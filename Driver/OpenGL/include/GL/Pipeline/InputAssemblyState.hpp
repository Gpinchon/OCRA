...
{
	const auto dynamicPrimitiveRestartEnable = a_DynamicState.Contains(PrimitiveRestartEnable);
	const auto dynamicPrimitiveTopology = a_DynamicState.Contains(PrimitiveTopology);
	if (dynamicPrimitiveRestartEnable && dynamicPrimitiveTopology)
		...
		return [primitivetopology(GetGLTopology(a_Info.topology))](Command::Buffer::ExecutionState& a_ExecutionState){
			a_ExecutionState.dynamicStates.primitiveRestartEnable ? glEnable(GL_PRIMITIVE_RESTART) : glDisable(GL_PRIMITIVE_RESTART);
			a_ExecutionState.renderPass.primitiveTopology = primitivetopology;
		};
	else if (dynamicPrimitiveTopology)
		return [primitiveRestartEnable(a_Info.primitiveRestartEnable)](Command::Buffer::ExecutionState& a_ExecutionState){
