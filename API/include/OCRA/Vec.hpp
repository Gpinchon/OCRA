...
struct Vec<2, T> {
    ...
    Vec<2, T>(const X& a_X, const Y& a_Y)
        : x(a_X)
        , y(a_Y)
    {
    }
...
struct Vec<3, T> {
    ...
    Vec(const X& a_X, const Y& a_Y, const Z& a_Z)
        : x(a_X)
        , y(a_Y)
        , z(a_Z)
    {
    ...
    template <typename U>
    Vec(const Vec<3, U>& a_V)
        : Vec(a_V.x, a_V.y, a_V.z)
    {
    }
    template <typename U>
    Vec(const Vec<2, U>& a_V)
        : Vec(a_V.x, a_V.y)
    {
    }
    auto& operator[](const size_t& a_Index) { return value[a_Index]; }
