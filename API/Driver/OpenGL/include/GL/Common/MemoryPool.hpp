...
public:
    ...
    class Deleter {
    public:
        constexpr Deleter(MemoryPool& a_Pool) : _memoryPool(a_Pool) {}
    ...
    constexpr MemoryPool(const MemoryPool& a_Other) noexcept : MemoryPool() {}
    template<typename U>
    constexpr MemoryPool(const MemoryPool<U>&) noexcept : MemoryPool() {}
    ~MemoryPool();
